...
Files_vp_hor=dir([input_directory '/' 'new_' 'vp_hor' 'model*.dat']); %file name
Files_vp_nmo=dir([input_directory '/' 'new_' 'vp_nmo' 'model*.dat']); %file name
...
for k=1:length(Files_vp)
    ...
    FileNames_vp_hor=Files_vp_hor(k).name;
    FileNames_vp_nmo=Files_vp_nmo(k).name;
...
%         disp([num2str(k) ' EXISTS===' FileNames_vp])
    ...
    true_model_vp_hor = dlmread([input_directory '/' FileNames_vp_hor]);
    true_model_vp_hor = reshape(true_model_vp_hor,nz,nx);
    
    true_model_vp_nmo = dlmread([input_directory '/' FileNames_vp_nmo]);
    true_model_vp_nmo = reshape(true_model_vp_nmo,nz,nx);
    ...
    smooth_model_vp_hor = true_model_vp_hor;
    for i = 1:num_smooth_iteration
        if i >= num_smooth_iteration-10
            smooth_model_vp_hor(1:water_depth,:) = true_model_vp_hor(1:water_depth,:);
        end
        smooth_model_vp_hor = imfilter(smooth_model_vp_hor, fspecial('gaussian',filter_size),'replicate','same');
    end
    smooth_model_epsi = ((smooth_model_vp_hor./smooth_model).^2-1)/2;
    smooth_model_epsi = reshape(smooth_model_epsi,1,nz*nx);

    
    
    smooth_model_vp_nmo = true_model_vp_nmo;
    for i = 1:num_smooth_iteration
        if i >= num_smooth_iteration-10
            smooth_model_vp_nmo(1:water_depth,:) = true_model_vp_nmo(1:water_depth,:);
        end
        smooth_model_vp_nmo = imfilter(smooth_model_vp_nmo, fspecial('gaussian',filter_size),'replicate','same');
    end
    smooth_model_delta = ((smooth_model_vp_nmo./smooth_model).^2-1)/2;
    smooth_model_delta = reshape(smooth_model_delta,1,nz*nx);
    
    true_model_epsi = ((true_model_vp_hor./true_model).^2-1)/2;
    true_model_delta = ((true_model_vp_nmo./true_model).^2-1)/2;
...
%     disp(name)
    ...
    %epsi
    if k/num_node ~= floor(k/num_node)
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node+1)) 'true' 'epsi' '.dat'],'wt');
    else
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node)) 'true' 'epsi' '.dat'],'wt');
    end
    
    disp(num2str(floor(k/num_node+1)))
    fprintf(fid,'%17.8f',true_model_epsi);
    fclose(fid);

    if k/num_node ~= floor(k/num_node)
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node+1)) 'mig' 'epsi' '.dat'],'wt');
    else
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node)) 'mig' 'epsi' '.dat'],'wt');
    end
    fprintf(fid,'%17.8f',smooth_model_epsi);
    fclose(fid);
    
    
    %delta
    if k/num_node ~= floor(k/num_node)
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node+1)) 'true' 'delta' '.dat'],'wt');
    else
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node)) 'true' 'delta' '.dat'],'wt');
    end
    
    disp(num2str(floor(k/num_node+1)))
    fprintf(fid,'%17.8f',true_model_delta);
    fclose(fid);

    if k/num_node ~= floor(k/num_node)
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node+1)) 'mig' 'delta' '.dat'],'wt');
    else
        fid=fopen([output_directory num2str(mod(k,num_node)+1) '/' num2str(floor(k/num_node)) 'mig' 'delta' '.dat'],'wt');
    end
    fprintf(fid,'%17.8f',smooth_model_delta);
    fclose(fid);
